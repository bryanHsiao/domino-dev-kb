...
| `Views/vwTestRealClone.view` | 仿真實來文簽辦單 view：selection 非 @All、分類最左欄、`$123` 遞減排序（`resort='ascending'`）、現行文字公式 |
...
| H. 文字比較 | vwTestRealClone | 預期報錯（官方："fails with a return code"） | **仿真實 view**：selection 非 @All + 分類最左欄 + $123 遞減排序 |
...
4. **若 DB 在伺服器上**：console 跑 `load updall <db path> -e` 建 design catalog（`'view'.column` 語法必要條件；之後改過 view 設計要再跑 `-d`）
5. Notes client 開該 DB → 動作選單 → `agtSeedTestDocs`（種資料）→ F9 重整 → `agtTestDQL`
6. 打開 `%TEMP%\dql-test-results.txt`，貼回給 Claude 對照預期矩陣分析
